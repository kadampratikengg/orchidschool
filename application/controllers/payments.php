...
class Payments extends CI_Controller {
	...
	public function save_bulkupload() {
		...
		if ($this->form_validation->run () == FALSE 
				|| $this->upload->do_upload('payments_excel') == FALSE) {
			...
			foreach ( $excel_data as $row ) {
				...
				$paid_invoices = $this->db->query("SELECT payment_id, students.student_firstname, admission_no
														FROM students, invoices, payments, standard_instalments 
														WHERE admission_no = '{$row[1]}' 
				...
				$query = $this->db->query("SELECT student_id FROM students 
											WHERE admission_no = '{$row[1]}'");


				if($query->num_rows()<1){
					$invalid_students[] = $row[1];
				}
			}	//end of foreach

			if(count($invalid_students)<1){
				...
				if($invoice_count < 1){
					...
					$paid_students = implode(", ",$paid_students);
					
					$this->session->set_flashdata('error_message', "Payment already added for these students = ".$paid_students);
					redirect ( base_url ( 'payments' ) );
					...
					foreach ( $excel_data as $row ) {
						...
						$late_fees = $row[2] - $payable_amount;
						...
						$data['narration'] 			= $row[4];
						$data['transaction_no'] 	= $row[3];
						$data['payment_date'] 		= date("Y-m-d", strtotime($row[0])); //swap_date_format($row[0]);
						$data['payment_amount'] 	= $payable_amount;
						$data['late_fee_amount'] 	= $late_fees;
						$data['total_paid_amount'] 	= $row[2];
				...
				$invalid_students = implode(", ",$invalid_students);
				
				$this->session->set_flashdata('error_message', "Invalid Admission No., Student not found for these admission no = ".$invalid_students);
				redirect ( base_url ( 'payments' ) );
			}
